...
@section('titulo')
    @lang('redimensionamento.configurarPivo')
@endsection
...
<div class="row col-12">
    <div class="text-center col-12">
        ...
        @else
            <a class="btn btn-outline-dark" href="{{route('status_afericao', $id_redimensionamento)}}">@lang('afericao.voltar')</a>
        @endif
    </div>
</div>
                    ...
                    <select name="lance_relativo"  class="form-control" required id="">
                        @foreach ($lances as $lance)
                            @if( !($lance['numero_lance'] == $numero_lances && $tem_balanco == 'sim') ) 
                                <option value="{{$lance['numero_lance']}}">  @lang('afericao.lance') {{$lance['numero_lance']}}  </option>                        
                            @endif      
                        @endforeach
                    </select>
                    <div class="line"></div>
...
<modal nome="removerLance" titulo="@lang('redimensionamento.removerLance')" css="modal-sm" >
    <formulario  id="formRemoverLance" css="row" action="{{route('removerLance')}}" method="delete" enctype="" token="{{ csrf_token() }}"  >
        <input type="hidden" name="id_lance" id="id_lance">      
        <input type="hidden" name="id_afericao" value="{{$id_redimensionamento}}">
        <div class="row col-12">
            <div class="col-12 container">
                <h5>@lang('redimensionamento.confirmarRemocaoLance')</h5> <h5 id="mensagem_remover"></h5>
            </div>
        </div>
        
    </formulario>
    <span slot="botoes">
        <button form='formRemoverLance' class="btn btn-lg btn-danger btn-block text-center text-light" style="margin: 0 auto" type="submit">@lang('unidadesAcoes.remover')</button>
    </span>
</modal>

<div hidden>
    <formulario  id="formEditarLance" css="row" action="{{route('getInformacoesLance')}}" method="put" enctype="" token="{{ csrf_token() }}">
