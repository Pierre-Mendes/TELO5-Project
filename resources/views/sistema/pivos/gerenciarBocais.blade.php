...
@section('titulo')
    @lang('bocais.bocaisEditar')
@endsection

@section('conteudo')

<div class="">
    <form action="{{route('bocais.edita')}}" method="POST">
        @csrf
        <input type="hidden" value="{{$fabricantes[0]['id_fabricante']}}" name="id_fabricante">
        ...
        <table class="table table-responsive-xl" id="tabelaTrechos">
            ...
            <tbody>                                
                @if(!empty($bocais))
                @foreach($bocais as $dado)
                    <tr>
                        <input type="hidden" name="id_bocal[]" value="{{$dado['id']}}" />
                        <td width="5%" class="form-control">{{$dado['id']}}</td>
                        <td><input type="text" name="fabricante[]" value="{{$dado['fabricante']}}" required class="form-control"></td>
                        <td><input  step="0.01" name="vazao_10_psi[]" value="{{$dado['vazao_10_psi']}}" required class="form-control"></td>
                        <td><input  name="intervalo_trabalho[]" value="{{$dado['intervalo_trabalho']}}" required class="form-control"></td>                          
        ...
        <div class="text-center">
            ...
            <a  id="" href="{{route('fabricantes.gerenciar', $fabricantes[0]['id_fabricante'])}}" class="btn btn-outline-dark"> @lang('unidadesAcoes.voltar')</a>
        </div>
    </form>
